...
ENGAGE_MINE_TIME = 7.0    # сек после engage -> считаем добытым (ресурс исчез, перс стоит)
HARD_CAP = 10.0           # ЖЁСТКИЙ потолок: максимум между добычами, сек
...
# --- Отбор целей (уверенность) ---
# Детектор уже фильтрует площадь/форму/окружение. Тут — доп. порог "уверенной"
# цели для клика. Металл мельче камня -> свой порог.
...
def pick_confident(targets):
    """Оставить только уверенные цели. Металл мельче -> свой порог."""
    def _ok(s):
        if s.get('type') == 'metal':
...
def wait_until_mined(stone_pos, dist_px, resource_type='stone'):
    """
    Ждём пока перс не добудет камень/металл — по ПИКСЕЛЯМ РЕСУРСА (камера статична).
    Возвращает (status, stats): status = 'mined'|'no_highlight'|'timeout'|'stop',
    stats = (base, lowest, last) — пиксели ресурса для лога/калибровки.

    Сигнал ИНВЕРТИРОВАН (нет глоу): base = пиксели ресурса в момент клика.
      px <= base*ENGAGE_DROP_FRAC -> перс пришёл, заслонил, ДОБЫВАЕТ
      px >= base*RECOVER_FRAC      -> ДОБЫТО (перс отошёл)
    resource_type: 'stone' или 'metal' — выбирает px-функцию.
    """
    px_fn = metal_px if resource_type == 'metal' else stone_px
    tx, ty = stone_pos
    start = time.time()
    ...
    bgr, _ = grab_screen()
    base = max(1, px_fn(bgr, tx, ty, STONE_GLOW_R))  # ресурс виден, перс идёт
    ...
    while time.time() < appear_deadline:
        ...
        bgr, _ = grab_screen()
        last = px_fn(bgr, tx, ty, STONE_GLOW_R)
    ...
    # Фолбэк: если ресурс исчез целиком (px=0 не восстанавливается) — ждём
    # ENGAGE_MINE_TIME сек после прихода перса, затем считаем добытым.
    ...
    while True:
        ...
        if time.time() > engage_time + ENGAGE_MINE_TIME:
...
def main():
    ...
    while True:
        ...
        target = None
        if all_targets:
            target = choose_target(all_targets, last_pos, anchor_px,
                                   recent_pts, blacklist_pts)

        # нет целей ИЛИ все в чёрном списке
        if target is None:
            reason = "целей нет" if not all_targets else "все цели в чёрном списке"
            if not PAN_ENABLED:
        ...
        print(f"[{clicks}] {rtype}@({cx},{cy}) area={target['area']} "
        ...
        if DRY_RUN:
            ...
            t0 = time.time()
            res, (gbase, glow, glast) = wait_until_mined(target['click'], d, rtype)
            took = time.time() - t0
            if res == 'stop':
                print("Стоп по клавише.")
                break
            print(f"    -> {res} за {took:.1f}s "
                  f"({rtype}_px base={gbase} min={glow} last={glast})")
            if res == 'no_highlight':
