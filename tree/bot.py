...
# Потолок выводим из размаха, чтоб НЕ обрывать обход до конца ромба.
# Полный туда-обратно = 2 * ряды * (длина_ряда + 1 шаг). +8 на пружину края.
MAX_EMPTY_PANS = 2 * PAN_NUM_ROWS * (PAN_ROW_LEN + 1) + 8  # стоп: обошли всё
# Диагональные направления (dx, dy): экран. Названия по сторонам света карты.
...
def pan_and_measure(bgr_before, mon, direction):
    """
    Перетянуть карту по диагонали direction (drag из центра монитора) и вернуть
    долю изменившихся пикселей (для лога).
    """
    if DRY_RUN:
        time.sleep(NO_TREE_WAIT)
        return 1.0
...
def main():
    ...
    empty_pans = 0     # счётчик панов подряд без целей
    # обход "газонокосилкой" по ДИАГОНАЛЯМ ромба, всё по счётчикам:
    #   PAN_ROW_LEN панов вдоль ряда (ось row) -> шаг на сосед. ряд (ось step) ->
    #   разворот ряда -> ... после PAN_NUM_ROWS шагов -> разворот оси step.
    row_dir = PAN_ROW_START    # 'SW'<->'NE'
    step_dir = PAN_STEP_START  # 'SE'<->'NW'
    row_pans = 0       # панов в текущем ряду
    step_count = 0     # шагов на сосед. ряд в текущем направлении
    pending_step = False
    ...
    def do_pan(bgr, mon, reason):
        ...
        if pending_step:
            # шаг на соседний ряд; после PAN_NUM_ROWS -> разворот оси шага
            direction = step_dir
            ...
            # едем вдоль ряда; после PAN_ROW_LEN -> шаг на соседний ряд
            direction = row_dir
            changed = pan_and_measure(bgr, mon, direction)
            row_pans += 1
            ...
            if row_pans >= PAN_ROW_LEN:
                row_pans = 0
                pending_step = True         # следующий пан = шаг на сосед. ряд
                row_dir = DIR_FLIP[row_dir]  # обратный ряд
                print(f"    -> ряд пройден: шаг '{step_dir}', обратно '{row_dir}'")

        last_pos = None           # карта сдвинулась -> координаты недействительны
        recent = []
        return False
    ...
    while True:
        ...
        # нет деревьев ИЛИ все в чёрном списке (ложные кусты) -> пан
        if target is None:
        ...
        empty_pans = 0   # есть реальная цель -> сбрасываем счётчик панов
